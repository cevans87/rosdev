...
class GenDockerSshPamEnvironment(Handler):
    ...
    async def resolve_options(cls, options: Options) -> Options:
        ...
        docker_container_volumes[options.docker_ssh_pam_environment_workspace_path] = (
            options.docker_ssh_pam_environment_container_path
        )
    ...
    async def validate_options(cls, options: Options) -> None:
        ...
        log.debug(
            f'docker_ssh_pam_environment_container_path: '
            f'{options.docker_ssh_pam_environment_container_path}'
        )
        log.debug(
            f'docker_ssh_pam_environment_workspace_path: '
            f'{options.docker_ssh_pam_environment_workspace_path}'
        )
    ...
    async def main(cls, options: Options) -> None:
        ...
        for k, v in (await GenEnvironment.get_environment_container(options)).items():
            ...
            options.write_text(
                path=options.docker_ssh_pam_environment_workspace_path,
                text='\n'.join(docker_ssh_pam_environment_lines)
            )
            await cls.exec_container(
                options=options,
                command=(
                    f'ln -f -s {options.docker_ssh_pam_environment_container_path} '
